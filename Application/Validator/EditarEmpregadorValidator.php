...
if (filter_input(INPUT_SERVER, 'REQUEST_METHOD') === 'POST') {
    if (filter_has_var($input, 'contactoE') && filter_input($input, 'contactoE') != '') {
        ...
        $pattern = "/^\[0-9]{4}(-\[0-9]{3})?$/";
        if (preg_match($pattern, $contato) === 0 && strlen($codPostal) !== 8) {
...
if (filter_input(INPUT_SERVER, 'REQUEST_METHOD') === 'POST') {
    $morada = filter_input($input, 'moradaE', FILTER_SANITIZE_STRING);
...
if (filter_input(INPUT_SERVER, 'REQUEST_METHOD') === 'POST') {
    $distrito = filter_input($input, 'distritoE', FILTER_SANITIZE_STRING);
...
if (filter_input(INPUT_SERVER, 'REQUEST_METHOD') === 'POST') {
     $concelho = filter_input($input, 'concelhoE', FILTER_SANITIZE_STRING);
